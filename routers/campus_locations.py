...
from schemas.campus_location import CampusLocationResponse, CampusLocationCreate, CampusLocationUpdate
...
async def list_campus_locations(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
...
async def get_campus_location(campus_location_id: int, db: Session = Depends(get_db)):
    ...
    campus_location = db.query(CampusLocation).filter(CampusLocation.id == campus_location_id).first()
...
async def create_campus_location(campus_location: CampusLocationCreate, db: Session = Depends(get_db)):
...
async def delete_campus_location(campus_location_id: int, db: Session = Depends(get_db)):
    # Retrieve the CampusLocation object by its ID
    campus_location_to_delete = db.query(CampusLocation).filter(CampusLocation.id == campus_location_id).first()
    ...
    if campus_location_to_delete:
        ...
        raise HTTPException(status_code=404, detail=f"CampusLocation with ID {campus_location_id} not found")


@router.put("/{campus_location_id}", response_model=CampusLocationResponse)
async def update_campus_location(campus_location_name: int, campus: CampusEnum,
                                 campus_location_data: CampusLocationUpdate, db: Session = Depends(get_db)):
    # Retrieve the CampusLocation object by its ID
    campus_location_to_update = db.query(CampusLocation).filter(
        CampusLocation.name == campus_location_name and CampusLocation.campus == campus).first()
